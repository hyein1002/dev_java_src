...
import com.util.DBConnectionMgr;//패키지가 다르니까 임포트해야함

//화면을 이용해서 리턴타입과 파라미터 복습하기
public class LoginForm implements ActionListener {
	
	Connection con = null;//물리적으로 떨어져있는 서버와 연결 통로 만들기
	PreparedStatement pstmt = null;//동적쿼리 작성하기 - ?
	//싱글톤 패턴을 이용하여 객체 주입 받기 - 한번 인스턴스화한걸 가지고 계속 나눠쓴다. 하나로 나누어쓴다.
	...
	public String[] login(String u_id, String u_pw) {
		...
		try {      
			sb.append(" SELECT  nvl((SELECT 1 FROM member 		");
			sb.append("             WHERE mem_id= ?),-1) status ");
			sb.append("  			FROM dual                   ");
...
//			pstmt.setString(2, u_pw);
			...
			if("1".equals(status)) {//아이디가 존재하니?       
				...
				rs = pstmt.executeQuery();
				if(rs.next()) {
					mem_name = rs.getString("mem_name");//sql문의 전달된 이름값
					db_id = u_id;
					result[0] = mem_name;//첫번째방에 이름을 담음//이름이나 에러메세지를 담음
					result[1] = db_id;//사용자가 입력한 id를 담음
				}
//아래에서 비밀번호가 맞지 않습니다. 메세지는 result배열에 담겨야 합니다.
//왜냐하면 리턴타입을 String에서 String[]로 바꾸었고 그 바뀐 정보를 if문에서 비교해야하니까
//더이상 mem_name에 값은 if문에서 역할이 없어진것이죠.
//비밀번호가 맞지 않습니다.를 result[0]에 담아주어야 합니다.
				if(mem_name.contentEquals("0")) {
			...
			//에러메세지를 JVM이 stack영역에 관리하는데 그 이력과 라인번호를 같이
			//출력해줌.
	...
	public void initDisplay() {
		//로그인 버튼을 눌렀을 때 이벤트 감지가 되면 actionPerformed 메소드를 호출하는데
		...
		jf_login.setSize(350, 250);
		jf_login.setVisible(true);
		jf_login.setTitle("로그인하세요");
	}
	//메인메소드
	...
	public void actionPerformed(ActionEvent e) {
		Object obj = e.getSource();//event가 일어난걸 감지
		//로그인 버튼을 누른거야?
		if(obj==jbtn_login) {
			...
			if(result[0].equals("비밀번호가 맞지 않습니다.")
				...
				//if문에서 return을 만나면 그 메소드를 탈출할 수 있다.
				return;
			}else{
				new BaseBallGameView(result);//생성자호출
				jf_login.dispose();
			}
			///hint는 account의 리턴값으로 담는다
			//game_seq는 cnt변수의 값을 담는다.
			///dap은 com배열의 값을 담는다.
			///input jtf_input.getText()
			///
		}
	}
}
