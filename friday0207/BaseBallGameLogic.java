...
public class BaseBallGameLogic {
	...
	public int history(BaseballVO bVO) {
        ...
        try{
        	...
        	//1row inserted
        	result = pstmt.executeUpdate();//위에서 작성한 insert문을 처리해주세요.
        	if(result==1) {//추가되는 row수를 받음
        		System.out.println("입력성공");
        	}else {
        		System.out.println("입력실패");//피드백 처리
        	}
    		}catch (Exception e) {
        	System.out.println("Exection"+e.toString());
		}
        return result;
	}                                                                       
	/********************************************
