...
public class CustomerBank extends JFrame {
	...
	public CustomerBank() {
		...
		if(mem_id==null) {
			return;//생성자 탈출 - 아래 메소드 2개는 호출이 안되요.//for문은 return
		}
		initDisplay();
		connect_process();
	}
	private void initDisplay() {
		jp_south.setLayout(new BorderLayout());//배치를 위한 설정 - 중앙과 남쪽으로 배치하려면 보더레이아웃을 사용해야한다.
	...
	public void connect_process() {
		try {
			...
			System.out.println("은행 서버에 접속할 수 없습니다.");
		}
	}
	//타임서버에 접속하기 구현
	...
	public static void main(String[] args) {
		CustomerBank cd = new CustomerBank();
	}
