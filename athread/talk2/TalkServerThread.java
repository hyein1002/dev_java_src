...
//			msg = st.nextToken();//뭐해?
			...
			for(TalkServerThread tst : ts.globalList) {
				...
				this.send(Protocol.LOGIN+Protocol.SEPERATOR+currentName);
...
//					st.nextToken();
					...
					for(TalkServerThread tst : ts.globalList) {
						if(otherName.equals(tst.chatName)) {//내가 선택한 상대가 맞는거야?
							tst.send(Protocol.ONE+Protocol.SEPERATOR+nickName+Protocol.SEPERATOR+otherName+Protocol.SEPERATOR+msg1);
						}
					}
					//그리고 나 자신에게도 전송해보자
					this.send(Protocol.ONE+"#"+nickName+"#"+otherName+"#"+msg1);
				}break;
				...
				case Protocol.CHANGE:{//대화명변경
					...
					broadCasting(Protocol.CHANGE+"#"+nickName+"#"+chatName);
