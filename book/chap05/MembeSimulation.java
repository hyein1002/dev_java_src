...
public class MembeSimulation {

	public static void main(String[] args) {
		
		//Member 클래스를 두개 만들 수 있는 방을 선언해봐요.
		Member mems[] = null;//아직은 방이 할당되지 않았어요. 찜은 되어있음.
		//선언한 객체배열을 생성하기 입니다.크기는 2로했어요
		//왜냐하면 이순신과 김유신 두 사람만 기억하면 되니까.....
		mems = new Member[2];//<-member세트를 두개 만듬. 이름만 2개 만든게 아니라 //생성하기
		//여기를 지나면 입장할 수 있어요.
		...
		mems[0] = mem;
		mem = null;//<-주소값을 없애서 아무것도 없게 만듬, 풍선줄을 끊어버림, 그렇지만 아직 주소값은 남아있어서 객체배열로 살릴수있나?
		//after
		...
		for(int i=0; i<mems.length;i++) {
			String id = mems[i].mem_id;//많이쓰는 패턴
