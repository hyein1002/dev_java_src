...
public class BaseballVO {
	...
	//setter메소드의 파라미터로 사용자가 입력한 값 혹은 선택한 정보 또는 결정된 값 들이 넘어온다.
	//그런데 이 값들은 지변에 담기게 되어서 메소드 영역밖에서는 사용이 불가함.
	//이 때 지변에 담긴 정보를 전변에 다시 초기화 해주면 클래스 전역에서 사용가능함.
	//또 내안이 아니더라도 다른 클래스에서 인스턴스화 하면 전변은 그대로 누릴 수 있다.
	public BaseballVO(int game_seq, String input,String hint, String dap, String mem_id) {
